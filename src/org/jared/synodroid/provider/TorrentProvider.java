...
public class TorrentProvider extends ContentProvider {
	...
	/**
	 * The content URI to use. Useful if the application have access to this
	 * class. Otherwise it must build the URI like<br/>
	 * <code>Uri uri = Uri.parse("content://org.transdroid.provider.torrents/search/eric%20Taix");</code>
	 * <br/>
	 ...
	 */
	@Override
	public Cursor query(Uri uriP, String[] projectionP, String selectionP,
			String[] selectionArgsP, String sortOrderP) {
		String[] columnNames = new String[] { "NAME", "URL", "SEEDERS",
				"LEECHERS", "SIZE" };
		...
		if (uriMatcher.match(uriP) == SEARCH_TERM) {
			...
			for (int tLoop = 0; tLoop < nb; tLoop++) {
				...
				values[0] = "" + linux[r.nextInt(3)] + "v"
						+ (r.nextInt(10) + 1) + ".0 (" + term + ")";
		...
		// Register to watch a content URI for changes (don't really know what
		// it means ?)
	 ...
	 */
	@Override
	public int update(Uri uriP, ContentValues valuesP, String selectionP,
