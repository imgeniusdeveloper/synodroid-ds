...
public class DownloadPreferenceActivity extends PreferenceActivity implements PreferenceProcessor {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    PreferenceCategory generalCategory = (PreferenceCategory) prefScreen.getPreferenceManager().findPreference(
            PREFERENCE_GENERAL);
    final ListPreferenceWithValue orderPref = ListPreferenceWithValue.create(this, "sort", R.string.label_process_sort,
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      ...
      case MENU_CREATE:
        ...
        PreferenceScreen screen = createServerPreference(maxServerId, serversCategory, PreferenceFacade.SERVER_PREFIX
                + maxServerId, getString(R.string.label_default_server_prefix) + maxServerId,
      ...
      case MENU_DELETE:
        ...
        builder.setPositiveButton(getString(R.string.button_delete), new OnClickListener() {
          public void onClick(DialogInterface dialog, int which) {
            ...
            serversCategory.removeAll();
            maxServerId = 0;
            PreferenceFacade.processLoadingServers(getPreferenceScreen().getSharedPreferences(),
  ...
  public void process(int idP, String keyP, Properties propertiesP) {
    String summary = buildURL(propertiesP.getProperty(PreferenceFacade.PROTOCOL_SUFFIX), propertiesP
            .getProperty(PreferenceFacade.HOST_SUFFIX), propertiesP.getProperty(PreferenceFacade.PORT_SUFFIX));
    if (idP > maxServerId) {
      maxServerId = idP;
    }
   ...
   */
  private PreferenceScreen createServerPreference(int idP, PreferenceGroup parentP, String keyP, String titleP,
          String summaryP) {
    ...
    //---- Nickname
    final EditTextPreferenceWithValue nickPref = EditTextPreferenceWithValue.create(this, keyP
            + PreferenceFacade.NICKNAME_SUFFIX, R.string.label_nickname, R.string.hint_nickname);
    nickPref.setText(titleP);
    nickPref.setDefaultValue(titleP);
    generalCategory.addPreference(nickPref);

    //---- Username
    generalCategory.addPreference(EditTextPreferenceWithValue.create(this, keyP + PreferenceFacade.USER_SUFFIX,
            R.string.label_username, R.string.hint_username));
    //---- Password
    generalCategory.addPreference(EditTextPreferenceWithValue.create(this, keyP + PreferenceFacade.PASSWORD_SUFFIX,
            R.string.label_password, R.string.hint_password).setInputType(
            InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD));
    //--- DSM Version
    generalCategory.addPreference(ListPreferenceWithValue.create(this, keyP + PreferenceFacade.DSM_SUFFIX,
            R.string.label_dsm_version, R.string.hint_dsm_version, DSMVersion.getValues()));
  ...
  private void addConnectionCategory(String keyP, PreferenceScreen screen, boolean showWifiP, int titleResIdP) {
    ...
    if (showWifiP) {
      ...
      final ListPreferenceWithValue wifiSSIDPref = ListPreferenceWithValue.create(this, keyP
              + PreferenceFacade.SSID_SUFFIX, R.string.label_wifissid, R.string.hint_wifissid, wifiSSIDs);
    ...
    //---- Protocol
    final ListPreferenceWithValue protocolPref = ListPreferenceWithValue.create(this, keyP
            + PreferenceFacade.PROTOCOL_SUFFIX, R.string.label_protocol, R.string.hint_protocol, SynoProtocol
            .getValues());
    connectionCategory.addPreference(protocolPref);
    //---- Host
    final EditTextPreferenceWithValue hostPref = EditTextPreferenceWithValue.create(this, keyP
            + PreferenceFacade.HOST_SUFFIX, R.string.label_host, R.string.hint_host);
    connectionCategory.addPreference(hostPref);
    //---- Port
    final EditTextPreferenceWithValue portPref = EditTextPreferenceWithValue.create(this,
            keyP + PreferenceFacade.PORT_SUFFIX, R.string.label_port, R.string.hint_port).setInputType(
...
//    });
    ...
    //-- Refresh value
    final EditTextPreferenceWithValue autoRefreshValue = EditTextPreferenceWithValue.create(this,
            keyP + PreferenceFacade.REFRESHVALUE_SUFFIX, R.string.label_refreshinterval, R.string.hint_refreshinterval)
            .setInputType(InputType.TYPE_CLASS_NUMBER);
    autoRefreshValue.setDefaultValue("15");
    connectionCategory.addPreference(autoRefreshValue);
    // Add dependencies. DON'T use 'setDependency()' when building Preferences at runtime
    connectionCategory.findPreference(keyP + PreferenceFacade.REFRESHVALUE_SUFFIX).setDependency(
  ...
  private String buildURL(String protoP, String hostP, String portP) {
    ...
    if ((protoP != null && protoP.length() > 0) || (hostP != null && hostP.length() > 0)
  ...
  /**
   * This method is called when the wizard finished. The metadata should contain information collected by the wizard. A
   * null paramter means that no information have been collected.
   * 
   * @param metaDataP
   */
  public void onWizardFinished(HashMap<String, Object> metaDataP) {
    if (metaDataP != null) {
      ...
      editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.HOST_SUFFIX, metaDataP.get(
              ServerWizard.META_HOST).toString());
      editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.REFRESHVALUE_SUFFIX, "5");
      editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.SSID_SUFFIX, metaDataP.get(
      ...
      if (((Boolean) metaDataP.get(ServerWizard.META_DDNS))) {
        ...
        editCommonsValues(editor, metaDataP);
        editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.HOST_SUFFIX, metaDataP.get(
                ServerWizard.META_DDNS_NAME).toString());
        editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.REFRESHVALUE_SUFFIX, "20");
        editor.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.SSID_SUFFIX, "");
    ...
    // Display a message for the end user
      ...
      AlertDialog.Builder builder = new AlertDialog.Builder(this);
      builder.setTitle(R.string.dialog_title_information).setMessage(R.string.wizard_no_server_found).setCancelable(
              false).setPositiveButton(R.string.button_ok, null).create().show();
    }
  }

  private void editCommonsValues(Editor editorP, HashMap<String, Object> metaDataP) {
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.NICKNAME_SUFFIX, metaDataP.get(
            ServerWizard.META_NAME).toString());
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.PROTOCOL_SUFFIX,
            ((Boolean) metaDataP.get(ServerWizard.META_HTTPS)) ? "HTTPS" : "HTTP");
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.PORT_SUFFIX, ((Boolean) metaDataP
            .get(ServerWizard.META_HTTPS)) ? "5001" : "5000");
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.DSM_SUFFIX, metaDataP.get(
            ServerWizard.META_DSM).toString());
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.USER_SUFFIX, metaDataP.get(
            ServerWizard.META_USERNAME).toString());
    editorP.putString(PreferenceFacade.SERVER_PREFIX + maxServerId + PreferenceFacade.PASSWORD_SUFFIX, metaDataP.get(
  ...
  private void showServerDialog(PreferenceScreen screenP) {
    ...
    Dialog dialog = new Dialog(this, TextUtils.isEmpty(title) ? android.R.style.Theme_NoTitleBar
